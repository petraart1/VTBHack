...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        try {
            ...
            String userId = claims.get("userId", String.class); // Извлекаем userId из claims
            ...
            // Используем email как principal, но сохраняем userId в details
            var auth = new UsernamePasswordAuthenticationToken(email, userId, authorities);
            SecurityContextHolder.getContext().setAuthentication(auth);

            log.debug("Authenticated user: email={}, userId={}, roles={}", email, userId, roles);
        } catch (Exception e) {
