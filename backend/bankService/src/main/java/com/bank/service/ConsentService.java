...
import com.bank.dto.request.ConsentRequestDto;
...
public class ConsentService {
    ...
    public ConsentResponseDto createConsentPublic(UUID userId, BankCredentials credentials, String clientId) {
        ...
        // Извлекаем teamXXX из clientId для requesting_bank
        String teamId = extractTeamIdFromClientId(clientId);
        ConsentRequestDto requestDto = ConsentRequestDto.createDefault(clientId, teamId);
        ...
        try {
            ...
            /**
     * Очистка кеша согласий для пользователя и банка
     */
    public void clearConsentCache(UUID userId, String bankId, String clientId) {
        String cacheKey = generateCacheKey(userId, bankId, clientId);
        consentCache.remove(cacheKey);
